import { DynamoDBClient, QueryCommand, PutItemCommand, ScanCommand, DeleteItemCommand, UpdateItemCommand, GetItemCommand, ResourceNotFoundException } from "@aws-sdk/client-dynamodb";
...
export class DbService {
    ...
    async saveNotificationStatus(userId, status) {
        const params = {
    ...
    async handleAccountDeletion(userId, userSub, password) {
        try {
            // 1. 先驗證密碼
    ...
    async beginDeletion(userId, userSub) {
        // 執行實際的刪除操作
    ...
    async markAsDeleted(table, userId) {
        const params = {
            TableName: table,
            Key: {
                userId: { S: userId }
    ...
    async deleteUserS3Files(userId) {
        try {
    ...
    async deleteUserCompletely(userId, userSub, password) {
        try {
            // 1. 驗證用戶存在性
            const isValid = await this.validateUserKeys(userId);
            if (!isValid) {
    ...
    // 補償機制
    async rollbackUserDeletion(userId) {
    ...
    async deleteAllUserRecords(userId) {
        try {
            // 1. 處理有排序鍵的表
            await Promise.all([
                this.deleteUserRecordsWithSortKey(userId, DB_TABLES.USER_ACTIVITY_LOG, 'timestamp'),
    ...
    async deleteUserRecordsWithSortKey(userId, table, sortKey) {
        try {
            const params = {
                TableName: table,
