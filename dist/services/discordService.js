...
export class DiscordService {
    ...
    // 驗證 Discord ID
    async verifyDiscordId(discordId) {
        try {
    ...
    // 發送 Discord 通知
    async sendNotification(discordId, type, title, content, link) {
        try {
            // 直接發送私人訊息
    ...
    // 新增私有的重試方法
    async sendNotificationWithRetry(webhookUrl, type, title, content, link, attempt = 1) {
        try {
            // 驗證 webhook URL
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    ...
                    if (retryAfter && attempt < DISCORD_CONFIG.RATE_LIMIT.MAX_RETRIES) {
                        ...
                        return this.sendNotificationWithRetry(webhookUrl, type, title, content, link, attempt + 1);
                    }
                }
                // 處理無效的 webhook
    ...
    async sendNotificationToUser(userId, type, title, content, link) {
        try {
            const params = {
    ...
    async sendDirectMessage(discordId, type, title, content, link) {
        try {
            if (!DISCORD_CONFIG.BOT_TOKEN) {
            ...
            // 1. 先建立 DM 頻道
            const createDMResponse = await fetch(`${DISCORD_CONFIG.API_ENDPOINT}/users/@me/channels`, {
                method: 'POST',
                headers: {
                    Authorization: `Bot ${DISCORD_CONFIG.BOT_TOKEN}`,
                    'Content-Type': 'application/json'
            ...
            const message = messageTemplate(title, content, link);
            // 3. 發送訊息到 DM 頻道
            const sendMessageResponse = await fetch(`${DISCORD_CONFIG.API_ENDPOINT}/channels/${dmChannel.id}/messages`, {
                method: 'POST',
                headers: {
                    Authorization: `Bot ${DISCORD_CONFIG.BOT_TOKEN}`,
                    'Content-Type': 'application/json'
