...
import { DynamoDBClient, PutItemCommand, ScanCommand, GetItemCommand, QueryCommand, DeleteItemCommand } from "@aws-sdk/client-dynamodb";
...
const prompts = {
    news: (url) => `請用繁體中文簡潔扼要地總結這篇 AWS 部落格文章的主要內容（限 100 字以內）：${url}`,
    announcement: (url) => `請用繁體中文簡潔扼要地總結這篇 AWS 公告的主要內容（限 100 字以內）：${url}`,
    knowledge: (url) => `請用繁體中文簡潔扼要地總結這篇 AWS Knowledge Center 文章的主要內容（限 100 字以內）：${url}`,
    solutions: (url) => `請用繁體中文簡潔扼要地總結這個 AWS Solution 的主要內容（限 100 字以內）：${url}`,
    architecture: (url) => `請用繁體中文簡潔扼要地總結這個 AWS Architecture 的主要內容（限 100 字以內）：${url}`
};
// 初始化客戶端
...
// 通用功能函數
async function checkIfExists(title, link, tableName) {
    const scanParams = {
        TableName: tableName,
...
async function summarizeContent(url, type) {
...
async function saveToDynamoDB(content, type, tableName) {
    // 提前檢查重複
    const exists = await checkIfExists(content.title, content.link, tableName);
    if (exists) {
        stats[type].skipped++;
        const { emoji } = CONTENT_TYPES[type];
...
// 爬蟲相關函數
async function gotoWithRetry(page, url, options = { waitUntil: "networkidle" }, retries = 3) {
    ...
    try {
        ...
        const articles = await page.evaluate((count) => {
            ...
            return Array.from(titles)
                .slice(0, count)
                .map((titleElem, index) => {
                const infoText = infos[index]?.innerText || "沒有資訊";
...
async function scrapeAnnouncement(browser) {
...
async function scrapeKnowledge(browser) {
    const context = await browser.newContext({
        locale: 'en-US',
        ...
        while (articlesScraped < FETCH_COUNTS.knowledge && hasNextPage) {
            ...
            try {
                ...
                if (currentPage > 1) {
                    ...
                    // 第一頁的訪問
                    await gotoWithRetry(page, 'https://repost.aws/knowledge-center/all?view=all&sort=recent', {
                        waitUntil: 'networkidle'
                    });
                }
                // 等待文章列表載入
                ...
                for (const article of articles) {
                    ...
                    try {
                        ...
                        await gotoWithRetry(articlePage, article.link, {
                            waitUntil: 'networkidle'
                        });
                        // 等待標題載入
...
async function scrapeSolutions(browser) {
        ...
        while (solutions.length < FETCH_COUNTS.solutions) {
            ...
            for (const cardInfo of cardsInfo) {
                ...
                try {
                    ...
                    const fullTitle = await page.$eval('.lb-breadcrumbs-dropTitle h1', (element) => element.textContent?.trim() || '沒有標題');
...
async function scrapeArchitecture(browser) {
    const page = await browser.newPage();
    const architectures = [];
    let currentPage = 1;
    try {
...
// 通知相關函數
async function sendNotifications(type, article, users) {
    try {
        const discordUsers = users.filter(user => user.discordNotification?.BOOL &&
            user.discordId?.S);
        if (discordUsers.length > 0) {
            ...
            for (const user of discordUsers) {
                if (!user.discordId?.S)
                    continue;
                try {
                    const success = await discordService.sendNotification(user.discordId.S, notificationType, article.title, article.summary, article.link);
                    if (success) {
...
async function getAllUserIds() {
    ...
    try {
        const command = new ScanCommand(params);
        const response = await dbClient.send(command);
        const userIds = response.Items?.map((item) => item.userId.S) || [];
...
async function addNotification(userId, contentId, category) {
    const params = {
...
async function broadcastNewContent(contentId, type) {
    try {
        ...
        for (const user of discordUsers) {
            ...
            try {
                const success = await discordService.sendNotification(user.discordId.S, notificationType, title, summary, link);
                if (success) {
                    stats[type].notifications.discord++;
                    logger.info(`成功發送 Discord 通知給用戶 ${user.userId.S}`);
                }
...
// 新增輔助函數來獲取內容詳細資訊
async function getContentDetails(contentId, type) {
...
// 修改日誌輸出格式
function logUpdateResult(type, result) {
    const { name, emoji } = CONTENT_TYPES[type];
    const total = result.inserted + result.skipped + result.failed;
    const totalNotifications = result.notifications.discord +
        result.notifications.email +
        result.notifications.line;
    const totalNotificationsFailed = result.notificationsFailed.discord +
...
// 修改進度追蹤函數
function logProgress(type, current, total, action) {
    const { name, emoji } = CONTENT_TYPES[type];
...
// 修改映射函數
function mapTypeToNotificationType(type) {
    const typeMap = {
...
async function getDiscordNotificationUsers() {
    try {
        const params = {
            TableName: "AWS_Blog_UserNotificationSettings",
...
// 修改主要的爬取函數
async function scrapeContent(browser, type, articles) {
    // 獲取啟用通知的用戶
...
export async function updateAllContent() {
    ...
    try {
        ...
        for (const task of tasks) {
            const { name, emoji } = CONTENT_TYPES[task.type];
        ...
        reportOrder.forEach(type => {
            logUpdateResult(type, stats[type]);
        });
...
//# sourceMappingURL=updateAllContent.js.map